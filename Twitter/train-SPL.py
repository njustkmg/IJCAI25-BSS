...
def assign_weights(net, train_dataset, device):
    ...
    evaluate_dataloader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, num_workers=32, pin_memory=True, drop_last=False)
    tokenizer = BertTokenizer.from_pretrained('/data/hlf/imbalance/unimodal/bert-base-uncased')
    ...
    with torch.no_grad():
        for step, (image, text, label) in enumerate(evaluate_dataloader):
            image = image.to(device)
            text = tokenizer(text, padding='longest', max_length=30, return_tensors="pt").to(device)
            label = label.to(device)
            ...
            cosine_sim = F.cosine_similarity(out_m1_, out_m2_, dim=1)
            similarities.extend(cosine_sim.detach().cpu().numpy())
            ...
            total_loss = loss_m1 + loss_m2 + loss_mm
            losses.extend(total_loss.detach().cpu().numpy())

    similarities_norm = normalize_min_max(similarities)
    losses_norm = normalize_min_max(losses)

    combined_scores = [(sim - loss) for sim, loss in zip(similarities_norm, losses_norm)]  # 相似度低 + 损失大 = 更难
...
if __name__ == '__main__':
    ...
    if True:
        ...
        for epoch in range(args.epochs):
            ...
            print('Now test_acc: %.4f || Now test_f1_macro: %.4f || Now test_f1_weighted: %.4f' % (test_acc, test_f1, f1_w))

            with open(results_file, 'a') as f:
                f.write(f'{epoch + 1} || {test_acc:.4f} {test_f1:.4f} {f1_w:.4f}\n')
